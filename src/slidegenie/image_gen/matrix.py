"""Matrix (table/comparison) image generator.

Ported from: ppt-addin/backend/services/make_pptx/image_gen/image/make_matrix.py
"""
...
class MatrixImageGenerator:
    def __init__(
        ...
        self.logger = logger
        self.prompt = prompt
        self.reference_json = reference_json or ""
    ...
    def build_image_prompt(self) -> str:
        template_name = resolve_image_prompt_template(
            ...
            addin_en_template="make_matriximage_en",
            addin_ja_template="make_matriximage_ja",
        )
        return load_prompt(
            template_name,
            prompt=self.prompt,
